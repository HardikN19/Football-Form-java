...
function checkSubmitBtn(){
    if(checkFirstName() & checkPhoneNumber() & checkUserName() & checkAgeGroup() & checkEmail() & checkCountry() & checkTeam() & checkPosition() & checkpinCode() & checkLastName()){
        ...
        if(btnSubmit.innerText=="Update data!"){
			updateData();
...
function updateData(){
	...
	for(let i =0;i<4;i++){
        if(position[i].checked){
            positionValue = position[i].value;
        }
    }
...
function callAjax() {
	...
	httpRequest.open('GET', `register?username=${userName.value}`);
